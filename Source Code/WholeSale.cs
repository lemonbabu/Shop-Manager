    ...
    public partial class WholeSale : Form
    {
        ...
        void confirmSale()
        {
            ...
            if (customerName != "" && customerPhone != "")
            {
                try
                {
                    obj.cmd = new SqlCommand("INSERT INTO tblCustomers (CustomerName, CustomerPhone, CustomerAddress, TotalCost, Discount, Payment, Due, PayMethod, Date, Time, WholeSale)  VALUES('" + customerName + "', '" + customerPhone + "', '" + customerAddress + "', '" + totalCost + "', '" + overallDiscount + "', '" + payment + "', '" + due + "', '" + payMethod + "', '" + date + "', '" + time + "', 'YES')", obj.con);
                    obj.con.Open();
                    obj.cmd.ExecuteNonQuery();
        ...
        private void txtProductSize_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dtbl.Rows.Count >= 1)
                {
                    ...
                    txtUnitPrice.Text = dtbl.Rows[0]["UnitWholeSalePrice"].ToString();
